...
def create_db(self):
    conn = sqlite3.connect('db_students1.db')
    with conn:
        cur = conn.cursor()
...
def first_run(self):
    data = ('Nick','Pranno','503-793-9601','npranno@gmail.com')
    conn = sqlite3.connect('db_students1.db')
    with conn:
        cur = conn.cursor()
        cur,count = count_records(cur)
        if count < 1:
            cur.execute("""INSERT INTO tbl_students (col_fname,col_lname,col_fullname,col_phone,col_email,col_course) VALUES (?,?,?,?,?,?)""", ('Nick','Pranno','Nick Pranno','503-793-9601','npranno@gmail.com','Math'))
            conn.commit()
    conn.close()
